(function () {
  ...
  function notifyFlashcardsChanged(reason, detail) {
    if (window.CT && typeof CT._emitDataUpdated === 'function') {
      CT._emitDataUpdated(reason || 'flashcards:update', Object.assign({
        concursoId: currentConcursoId()
      }, detail || {}));
    }
    if (window.CT && typeof CT.queueAutoSave === 'function') {
      CT.queueAutoSave(reason || 'flashcards:update', { delay: 700 });
    }
  }

  function write(key, value) {
    localStorage.setItem(key, JSON.stringify(value));
    if (key === KEYS.decks || key === KEYS.cards || key === KEYS.log) {
      notifyFlashcardsChanged('flashcards:save', { key: key });
    }
  }
  ...
  function appendLog(item) {
    ...
    write(KEYS.log, log);
    if (window.CT && typeof CT._emitDataUpdated === 'function') {
      CT._emitDataUpdated('flashcards:review', {
        concursoId: item && item.concursoId,
        data: item && item.data,
      });
    }
  }
  ...
  function refreshStatsSummary() {
    ...
    var statsGrid = root.querySelector('.flash-stat-grid');
    if (statsGrid) statsGrid.outerHTML = renderStats(calcStats());
    updateTabBadge();
  }
  ...
  function refreshCurrentView() {
    ...
    var view = document.getElementById('flashMainView');
    if (view) {
      view.innerHTML = state.activeDeckId ? renderDeckDetail(state.activeDeckId) : renderDecksView();
    }
  }
  ...
  function renderDeckDetail(deckId) {
    ...
    return [
      ...
      '      <button class="flash-btn secondary" id="btnSyncAnki" onclick="FlashcardsDashboard.syncAnkiConnect(\'' + deckId + '\')" style="color: #4f8ef7; border-color: #4f8ef7;">🔄 Sync Anki 1</button>',
  ...
  async function deleteCard(cardId) {
    if (!(await CT.confirm('Excluir este flashcard?', {
  ...
  function showDashTab(tab) {
    ...
    document.documentElement.classList.remove('ct-initial-flashcards');
    if (document.body) document.body.classList.add('ct-dashboard-mounted');
 ...
 async function syncAnkiConnect(localDeckId) {
    if (!window.pywebview || !window.pywebview.api) {
      toast('Aguarde o carregamento do sistema.', '!');
      return;
    }
    
    if (!localDeckId) {
      showFlashAlert('Por favor, entre em um baralho do Track Concursos primeiro para poder importar os cards para ele.', 'Atenção');
      return;
    }

    var btn = document.getElementById('btnSyncAnki');
    var textoOriginal = btn ? btn.innerHTML : 'Sync Anki';
    if (btn) { btn.innerHTML = '⏳ Buscando Baralhos...'; btn.disabled = true; }

    try {
      var resultado = await window.pywebview.api.sync_anki();
      if (resultado.ok) {
        // Se conectou, abre o modal enviando a lista de baralhos do Anki
        abrirModalAnki(resultado.decks, localDeckId);
      } else {
        showFlashAlert(resultado.motivo, 'Erro no AnkiConnect');
      }
    } catch (err) {
      showFlashAlert('Erro ao tentar comunicar com o sistema local.', 'Erro');
    } finally {
      if (btn) { btn.innerHTML = textoOriginal; btn.disabled = false; }
    }
  }

  async function enviarParaAnki(localDeckId) {
    // 1. Pede o nome do baralho de destino no Anki
    var ankiDeck = prompt("Digite o nome do baralho do Anki para onde deseja enviar os cards:");
    if (!ankiDeck) return; // Se cancelar, aborta

    // Pode usar a função toast do seu app para avisar que começou
    if (window.CT && typeof CT.toast === 'function') {
        CT.toast('Processando e enviando para o Anki...', 'Aguarde');
    }

    // 2. Puxa todos os cards do Track Concursos
    var ctCards = JSON.parse(localStorage.getItem('ct_flashcards') || '[]');
    
    // 3. Filtra apenas os cards que pertencem ao baralho que você está exportando
    var cardsDesteBaralho = ctCards.filter(c => c.deckId === localDeckId);
    
    if (cardsDesteBaralho.length === 0) {
        alert('Nenhum card encontrado neste baralho local.');
        return;
    }

    // 4. Limpa os dados, mandando apenas Frente e Verso para o Python
    var cardsParaExportar = cardsDesteBaralho.map(c => ({
        front: c.front || "",
        back: c.back || ""
    }));
    
    // 5. Envia para a ponte do pywebview
    try {
        var resultado = await window.pywebview.api.export_to_anki(ankiDeck, cardsParaExportar);
        
        if (resultado.ok) {
            alert('✅ Sincronização concluída: ' + resultado.msg);
        } else {
            alert('❌ Erro na Exportação: ' + resultado.msg);
        }
    } catch (err) {
        alert('Falha crítica na comunicação com o sistema local Python.');
    }
  }

  function abrirModalAnki(decks, localDeckId) {
    // Monta as opções do Datalist para permitir pesquisa
    var optionsHtml = decks.map(d => `<option value="${d}">`).join('');

    var modalHtml = `
      <div id="ankiImportModal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); z-index: 9999; display: flex; justify-content: center; align-items: center;">
        <div style="background: #1e1e2e; padding: 25px; border-radius: 8px; width: 450px; color: white; box-shadow: 0 4px 15px rgba(0,0,0,0.3);">
          <h3 style="margin-top: 0; color: #4f8ef7;">Importar do Anki</h3>
          <p style="font-size: 14px; color: #ccc;">Selecione qual baralho do Anki você deseja importar para cá:</p>
          
          <input list="ankiDecksList" id="ankiDeckInput" placeholder="Digite para buscar entre os ${decks.length} baralhos..." style="width: 100%; padding: 10px; margin-bottom: 20px; border-radius: 5px; border: 1px solid #4f8ef7; background: #2a2a3b; color: white; box-sizing: border-box;">
          <datalist id="ankiDecksList">
              ${optionsHtml}
          </datalist>

          <div style="display: flex; justify-content: flex-end; gap: 10px;">
            <button onclick="document.getElementById('ankiImportModal').remove()" style="padding: 8px 15px; background: transparent; border: 1px solid #666; color: #ccc; border-radius: 5px; cursor: pointer;">Cancelar</button>
            <button id="btnConfirmarAnki" onclick="FlashcardsDashboard.confirmarImportacaoAnki('${localDeckId}')" style="padding: 8px 15px; background: #4f8ef7; border: none; color: white; border-radius: 5px; cursor: pointer; font-weight: bold;">📥 Puxar Cards</button>
          </div>
        </div>
      </div>
    `;

    var oldModal = document.getElementById('ankiImportModal');
    if (oldModal) oldModal.remove();
    document.body.insertAdjacentHTML('beforeend', modalHtml);
  }

  async function confirmarImportacaoAnki(localDeckId) {
    var ankiDeck = document.getElementById('ankiDeckInput').value;
    if (!ankiDeck) {
      toast('Selecione ou digite o nome de um baralho do Anki!', '!');
      return;
    }

    var btn = document.getElementById('btnConfirmarAnki');
    btn.innerHTML = '⏳ Importando (isso pode demorar)...';
    btn.disabled = true;

    try {
      var resultado = await window.pywebview.api.import_anki_cards(ankiDeck, localDeckId);
      
      if (resultado.ok && resultado.cards) {
        // 1. Puxa os dados reais do local storage
        var ctCards = JSON.parse(localStorage.getItem('ct_flashcards') || '[]');
        var ctDecks = JSON.parse(localStorage.getItem('ct_flashcard_decks') || '[]');
        
        // 2. Encontra o baralho atual para herdar concursoId, nome e path
        var deckAtual = ctDecks.find(d => d.id === localDeckId) || {};
        var concursoId = deckAtual.concursoId || "";
        var deckName = deckAtual.name || "";
        var deckPath = deckAtual.path || "";
        
        var agora = new Date().toISOString();

        // 3. Molda os novos cards no padrão EXATO do Track Concursos
        resultado.cards.forEach(card => {
           // Gera ID compatível (fccard_ + string)
           var randomStr = Math.random().toString(36).substring(2, 10);
           var novoId = 'fccard_' + Date.now().toString(36) + randomStr;
           ...
           var novoCard = {
              ...
              "answer": "",
              "explanation": "",
              "tags": ["Importado do Anki"], // Tag para facilitar a organização depois
              "updatedAt": agora,
              "deckName": deckName,
              "deckPath": deckPath
           };
           
           ctCards.push(novoCard);
        });

        // 4. Salva de volta no sistema local
        localStorage.setItem('ct_flashcards', JSON.stringify(ctCards));
        
        // 5. Força o Track Concursos a salvar o backup no arquivo JSON geral
        if (window.CT && typeof CT.markSavedState === 'function') {
           CT.markSavedState();
        }

        document.getElementById('ankiImportModal').remove();
        
        // 6. Recarrega a tela 
        alert(resultado.msg + ' A tela será recarregada para visualizar os novos cards.');
        window.location.reload(); 
        
      } else {
        showFlashAlert(resultado.msg || 'Erro desconhecido', 'Erro na Importação');
        btn.innerHTML = '📥 Puxar Cards';
        btn.disabled = false;
      }
    } catch (err) {
      showFlashAlert('Erro de comunicação com o Python durante a importação.', 'Erro');
      btn.innerHTML = '📥 Puxar Cards';
      btn.disabled = false;
    }
  }
  ...
  window.FlashcardsDashboard = {
    render: render,
    syncAnkiConnect: syncAnkiConnect,    
    confirmarImportacaoAnki: confirmarImportacaoAnki,
